...
export default function AdminPage() {
    ...
    return (
        <div className={styles.adminContainer}>
            <h1 className={styles.adminTitle}>Админка</h1>
                ...
                <table className={styles.table}>
                    ...
                    <tbody>
                        {users.map(user => (
                            <tr key={user.id}>
                                <td>{user.id}</td>
                                <td>{editingUserId === user.id ? <input type="text" name="username" value={editedData.username} onChange={handleInputChange} className={styles.editInput}/> : user.username}</td>
                                <td>{editingUserId === user.id ? <input type="email" name="email" value={editedData.email} onChange={handleInputChange} className={styles.editInput}/> : user.email}</td>
                                ...
                                <td>
                                    {editingUserId === user.id ? (
                                        <>
                                            <button onClick={() => handleSave(user.id)} className={styles.saveButton}>Сохранить</button>
                                            <button onClick={handleCancelEdit} className={styles.cancelButton}>Отмена</button>
                                        </>
                                    ) : (
                                        <>
                                            <button onClick={() => handleEditClick(user)} className={styles.editButton}>Редактировать</button>
                                            <button onClick={() => handleDelete(user.id)} className={styles.deleteButton}>Удалить</button>
                                        </>
                                    )}
                                </td>
                            </tr>
            ...
            <div className={styles.newsForm}>
                <input type="text" name="title" placeholder="Заголовок" value={newPost.title} onChange={handleNewPostChange}/>
                <textarea name="content" placeholder="Содержание" value={newPost.content} onChange={handleNewPostChange}/>
                <input type="file" name="media" onChange={handleNewPostChange}/>
                ...
                <button onClick={handleCreatePost}>Создать новость</button>
            </div>
                    ...
                    <tbody>
                        {news.map(n => (
                            <tr key={n.id}>
                                ...
                                <td><button onClick={() => handleDeleteNews(n.id)} className={styles.deleteButton}>Удалить</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
